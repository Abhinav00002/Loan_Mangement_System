...
public	Account findBybankId(Integer accountId);
	...
	public List<Map<String, Object>> getTotalDue(@Param("toDate") LocalDate toDate, @Param("fromDate") LocalDate fromDate);
...
// get damp of all data
	@Query(value="SELECT"
			...
			+ "    inner join scheme_master scm ON lm.scheme=scm.scheme_id "
			+ "WHERE"
			+ "    lrm.preclose = 0"
			+ "     and lrm.due_date between :toDate and :fromDate "
			/*+ "GROUP BY"
			+ "    lm.loan_id"*/,nativeQuery = true)
	public List<Map<String, Object>> getTotalDamp(@Param("toDate") LocalDate toDate, @Param("fromDate") LocalDate fromDate);
